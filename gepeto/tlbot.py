...
def record_all_messages_decorator(func):
...
async def echo_private(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        ## save bot_res
        with open("recorded_messages.tx", "a") as f:
            f.write(f"\nChat ID: {message.chat_id}, Message ID: {message.message_id+1}, Text: {bot_res}")
...
if __name__ == '__main__':
    ...
    reply_when_mentioned = MessageHandler(filters.Entity('mention') | (filters.REPLY), reply_when_mentioned)
